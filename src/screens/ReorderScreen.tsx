  ...
  useEffect(() => {
    const loadSchedule = async () => {
      ...
      if (error) {
        ...
        const workoutList = sorted.map((item) => ({
          id: `${item.date}_${item.workout_id}`, // composite key
  ...
  const handleSave = async () => {
    ...
    for (let i = 0; i < dates.length; i++) {
      ...
      const { error } = await supabase
        ...
        .update({
          workout_id: workout.workout_id, // now this is a UUID
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#f8fafc" }}>
      ...
      <Text style={styles.headerSubtitle}>
        Drag workouts to rearrange your schedule
      </Text>
      ...
      <DraggableFlatList
        ...
        renderItem={({ item, drag, isActive }) => {
          const idx = workouts.findIndex((w) => w.id === item.id);
          return (
            <TouchableOpacity
              ...
              <View style={{ flex: 1 }}>
                <View style={styles.cardRow}>
                  ...
                  <Text style={styles.cardDay}>
                    {(() => {
                      try {
                        return dates[idx]
                          ? format(parseISO(dates[idx]), "EEE dd")
                          : "—";
                      } catch {
                        return "—";
                      }
                ...
                <View style={styles.cardMetaRow}>
                  ...
                  <Text
                    style={[
                      styles.cardTag,
                      { backgroundColor: item.tagBg, color: item.tagColor },
                    ]}
                  >
                    {item.difficulty}
                  </Text>
      ...
      <View style={styles.instructions}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.instructionsText}>
            Touch and hold the grip lines (≡) on the right side of each workout
            card, then drag up or down to rearrange your schedule.
          </Text>
        </View>
      </View>
